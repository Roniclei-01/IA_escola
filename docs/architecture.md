...
- `Library`: navegacao por categorias/subcategorias, importacao e gerenciamento de materiais.
...
- `Categories`: manutencao dedicada de categorias e subcategorias de estudo.
...
A UI usa esses dados para exibir historico visual por documento, uma fila de cards vencidos, navegacao inicial por categorias academicas, subcategorias e caminho curto da biblioteca, pagina principal dedicada a biblioteca/importacao, pagina secundaria para o `Estudo ativo` aberta apos importar ou selecionar um livro, modal de importacao com classificacao obrigatoria antes de subir o arquivo, modal `Meus Livros` filtrado pelo contexto atual, filtros, busca textual, ordenacao da biblioteca, comparativo de progresso entre documentos, classificacao de estudo editavel por documento, resumo filtravel de metricas por periodo, metas persistidas e recorrentes de revisao por documento com alerta visual de pendencia, painel de idioma original recolhivel por seta, marcador automatico da pagina de leitura para retomar o estudo, modal de `Anotacao` acionado por `+` no painel traduzido com adicionar, editar e excluir entradas, preferencia de lembretes, horario configuravel e notificacao nativa agendada do Tauri quando aplicavel, resumo de desempenho por sessao, tendencia temporal de retencao, evolucao de cards dificeis por periodo, exportacao local de relatorio Markdown, previa e relatorio imprimivel em PDF com paginacao, grafico de desempenho e tendencia por sessao, exportacao de cards para Anki em `.apkg` pela interface, TSV com diretivas de importacao, GUID estavel e tags de origem, e geracao local inicial de pacote Anki `.apkg` no backend.
...
- Criar UI minima com biblioteca/importacao na tela principal e estudo em pagina secundaria.
- Navegar a biblioteca por categoria/subcategoria antes da lista de livros.
