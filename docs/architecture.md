...
A UI usa esses dados para exibir historico visual por documento, uma fila de cards vencidos, filtros, busca textual, ordenacao da biblioteca, comparativo de progresso entre documentos, resumo filtravel de metricas por periodo, metas persistidas e recorrentes de revisao por documento com alerta visual de pendencia e notificacao local quando aplicavel, resumo de desempenho por sessao, tendencia temporal de retencao, evolucao de cards dificeis por periodo, exportacao local de relatorio Markdown, previa e relatorio imprimivel em PDF com paginacao, grafico de desempenho e tendencia por sessao, e exportacao de cards para Anki em TSV com diretivas de importacao e tags de origem.
