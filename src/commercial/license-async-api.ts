...
async function handlePaddleWebhookRequest(
  request: CommercialApiRequest,
  backend: AsyncCommercialLicenseBackend,
  ...
  const result = await handlePaddleWebhookAsync(request.body ?? "", signature, verifier, backend);
...
async function handleLicenseActivationRequest(
  request: CommercialApiRequest,
  backend: AsyncCommercialLicenseBackend
): Promise<CommercialApiResponse> {
  ...
  const result = await backend.activateLicense(activationRequest);
